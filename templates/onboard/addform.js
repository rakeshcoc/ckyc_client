 ...
 if(currentTab == 1){
    ...
    var main_boolean = true;
     for(var i =0 ; i < alldivs.length; i++){
        var span_elements = alldivs[i].childNodes
        var sub_boolean = false
        for(var j=0; j < span_elements.length;j++){
          var input_ele = span_elements[j].childNodes;
          for(var temp_index =0 ; temp_index < input_ele.length;temp_index++){
            if(input_ele[temp_index].id != null){
              if(input_ele[temp_index].checked == true){
                console.log(input_ele[temp_index].id + span_elements[j]);
                sub_boolean = true
                break;
              }

              // else{
              //   //console.log(input_elements[j].id+" :False"+ sub_boolean); 
              //   sub_boolean = false
              // }  
          }
        } 
        ...
        console.log("main_boolean", main_boolean , sub_boolean)
     }
  if(!main_boolean){
...
function getCheckedCheckboxesFor() {
    ...
    for(var i=0; i < checkboxes.length; i++ ){
      ...
      for(var k = 0; k < elms.length; k++){
        var x = elms[k].parentElement.parentNode.id;
        console.log("sample123: "+ x)
        if(proof_Map.has(x)){
