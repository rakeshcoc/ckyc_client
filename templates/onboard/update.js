...
function validateForm() {
  ...
  if(currentTab == 0){
    ...
    var input_divs = document.getElementsByClassName("up-class");
    console.log("input_divs.length"+input_divs.length)
    for(var temp_index =0; temp_index< input_divs.length;temp_index++){
      console.log("in page1:" + input_divs[temp_index].value);
      if(input_divs[temp_index].value != ''){
        page_boolean = true
        //console.log("updivs[temp_index].value"+input_divs[temp_index].value)
        break;
      }
    }
    if(!page_boolean){
  ...
  if(currentTab == 1){
    var alldivs = document.getElementsByClassName("sample_div");
    var main_boolean = true;
     for(var i =0 ; i < alldivs.length; i++){
        var input_elements = alldivs[i].childNodes
        var sub_boolean = true
        for(var j=0; j < input_elements.length;j++){
          if(input_elements[j].id != null){
            if(input_elements[j].checked == true){
              console.log(input_elements[j]+" : Ture");
              sub_boolean = sub_boolean || sub_boolean
          }
          else{
             console.log(input_elements[j]+" :False"); 
             sub_boolean = false
          }
        ...
        main_boolean = main_boolean && sub_boolean
     }
  if(!main_boolean){
    alert("Atleast one should be selected in each section")
  }
  valid = main_boolean
  }
  for (i = 0; i < y.length; i++) {
    // If a field is empty...
    if (y[i].value == "" && y[i].disabled !=true && y[i].placeholder != 'Optional' && y[i].placeholder != 'Required' ) {
